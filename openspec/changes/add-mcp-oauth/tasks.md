...
- [x] 5.1 Extend or add `scripts/e2e-mcp-oauth.ts`: discovery → (DCR or static)
      → PKCE → `schedule_post` on disposable Postgres/Redis; keep existing
      API-key MCP E2E green.
- [x] 5.2 Update `docs/specs/SPEC_API_MCP.md` (OAuth/MCP sections only; do not
      resurrect human JWT), architecture flows, STATUS backlog, CHANGELOG.
- [x] 5.3 Run focused tests plus `bun run check`, `bun run db:check`,
