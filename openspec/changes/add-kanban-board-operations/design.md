...
## Package boundaries and why this is entirely a web change

Nothing here belongs below `apps/web`. Every operation the board performs already exists as a
...
None are regenerated by this change: no API contract changes here. (The publication feed's two new
fields belong to `add-home-operational-blocks`.)

## Compatibility and rollback

The five columns, their derivation and the existing failed-to-scheduled retry are preserved, so a
...
needs undoing, and a stale `mp-kanban-prefs` entry in a browser is inert.
