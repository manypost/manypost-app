    ...
    <div className="inline-flex items-center gap-1 rounded-md border border-line bg-surface p-1" role="group">
      {(['MONTHLY', 'YEARLY'] as const).map((period) => (
        <button
          ...
          className={cn(
            'flex items-center gap-1.5 rounded-sm px-3 py-1.5 text-meta font-semibold outline-none transition-colors duration-200',
        ...
        >
          ...
          {period === 'YEARLY' ? (
            <span className="rounded-sm bg-accent px-1.5 py-0.5 text-axis font-semibold text-paper">
              {t('yearlyDiscount')}
            </span>
          ) : null}
        </button>
    ...
    <button
      ...
      className={cn(
        ...
        selected
          ...
          : 'bg-surface hover:border-accent hover:bg-surface-2',
      )}
    >
      <span className="flex w-full items-center gap-1.5">
        <span className="text-compact font-semibold text-ink">{PLAN_LABEL[tier]}</span>
        {current ? (
      ...
      <span className="flex items-baseline gap-1">
        ...
        <span className="text-meta text-graphite">{price === null ? '' : t('perMonth')}</span>
      </span>

      <span className="text-meta leading-snug text-mist">
        {price === null
          ? t('freeForever')
          : period === 'YEARLY'
...
export function PlanIncludes({ tier }: { tier: PlanTier }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <h3 className="flex items-center gap-2 text-compact font-semibold text-ink">
        {tier === 'FREE' ? (
