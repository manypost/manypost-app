...
export function ChannelsPanel({
  ...
  return (
    <>
      {/* --- MOBILE ONLY (< lg): Barra compacta + seletor horizontal de canais --- */}
      <div className="flex flex-col gap-2.5 rounded-lg border border-line bg-surface p-3 lg:hidden">
        <div className="flex items-center gap-2">
          <Button
            size="sm"
            className="flex-1 gap-1.5 font-semibold text-paper"
            ...
            <div className="flex items-center justify-between">
              ...
              {selectedIds.length > 0 ? (
                <button
                  type="button"
                  onClick={onClear}
                  className="cursor-pointer text-meta font-semibold text-accent transition-colors hover:underline"
                >
              ...
              {channels.data!.map((ch) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="max-w-36 truncate text-meta sm:text-compact">{ch.name ?? ch.username ?? ch.id}</span>
                    {needsAttention ? (
      ...
      <aside className="hidden h-fit max-h-[calc(100dvh-7rem)] w-full shrink-0 flex-col gap-3 rounded-lg border border-line bg-surface p-4 lg:sticky lg:top-20 lg:flex lg:w-64">
        <h2 className="text-panel font-semibold tracking-[-0.2px] text-ink">{t('title')}</h2>
        <div className="flex gap-2">
          <Button
            size="sm"
            className="flex-1 gap-1.5 text-paper"
            onClick={() => openComposer()}
          >
