...
>
> **Estado de verdade (2026-07-26):** posse por item (`publication_attempts`), semáforo `maxConcurrent` via Redis e rethrow de erro inesperado em publish/thread estão no código. Anti-SSRF com **pin de DNS na conexão** está em `outbound-request-security` (living). Confirme em `publishing.ts`, `packages/queue` e `openspec/specs/`.
