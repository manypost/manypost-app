...
>
> **Última atualização:** 2026-07-12 (fatias de aprovação por link, listagens/SSE/retry e providers onda 1: Telegram + Bluesky) · branch `main`.
...
| **Providers** ⭐ | Onda 1: **Mastodon** (OAuth por instância, `MASTODON_DEFAULT_INSTANCE` opcional pré-preenche), **Telegram** (bot; conecta por `@canal`/id, valida admin+can_post via getChat/getChatMember; publish sendMessage/Photo/Video/MediaGroup por URL pública; réplicas via `reply_parameters`; token do bot cifrado no canal), **Bluesky** (app password sobre AT Protocol; createSession→refreshSession por publish; imagens via uploadBlob+embed; thread com root/parent corretos via getPosts; PDS custom por `service`). **Fake** p/ E2E. Todos passam o **test-kit de contrato** (`test-kit/contract.ts`: schemas, classifyError, validateMedia, threads→publishReply) | SPEC_INTEGRATIONS §2/§7 | `packages/providers/src/{mastodon,telegram,bluesky,fake}` |
...
**Provas:** 137 testes unit (`bun test`, inclui golden bodies de Telegram/Bluesky com fetch mockado + test-kit de contrato aplicado a todo provider) + `scripts/e2e-auth.ts` (catálogo `/providers` filtrado por env: bluesky/mastodon sempre, telegram só com token, provider sem env → connect 404) + `scripts/e2e-publish.ts` (106 checks) — CI roda tudo com services postgres+redis. **Smoke real opcional**: `scripts/live-telegram.ts` (`TG_CHAT=@canal bun run scripts/live-telegram.ts` publica de verdade — não roda no CI).
...
5. **Providers onda 1**: ~~Mastodon~~ ✅, ~~Telegram~~ ✅, ~~Bluesky~~ ✅ (2026-07-12) + test-kit de contrato ✅. **Faltam Discord, LinkedIn, X** (X = traga-sua-chave, DECISIONS §6). Env já preparado (`DISCORD_*`/`LINKEDIN_*`/`X_*`); catálogo já esconde quem não tem env. Discord: bot token + webhook por canal; LinkedIn: OAuth `w_member_social` (+página exige parceria); X: OAuth2 PKCE + chunked upload de mídia. Guia de credenciais em [INTEGRATIONS_SETUP.md](INTEGRATIONS_SETUP.md). Postiz ref: `libraries/nestjs-libraries/src/integrations/social/*.provider.ts`. **Onda 1 restante NÃO tem gate** (Discord/LinkedIn member) — X depende só da chave do usuário.
