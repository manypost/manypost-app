...
| | |
|---|---|
| **Última entrega** | Onda 15 (2026-07-23) — **Instagram (standalone)**, 2º provider da família Meta (Instagram Login, sem Página do Facebook): foto/reel/carrossel/story + réplicas como comentários, molde container→poll→publish do Threads. Antes, onda 14: profundidade 3D sem sombra (brand v1.3) |
| **Fase** | Fase 1 (MVP): backend completo e verificado, web com toda a superfície da API, billing do Cloud entregue |
| **Provas** | `bun run check` verde — **365 testes** + typecheck (api/web) + fronteiras + grep de IA + brand. E2E reais no CI: `e2e-auth`, `e2e-publish`, `e2e-public`, `e2e-mcp`, `e2e-billing` |
| **Redes prontas** | Mastodon, Bluesky, Telegram, Discord (OAuth2+Bot **e** webhook), LinkedIn, X, TikTok (sandbox — auditoria em revisão), Threads e **Instagram standalone** (Development Mode), Twitch e Kick (chat ao vivo) + `fake` para testes |
| **Próxima onda** | Resto da família Meta: **Facebook Pages** e o **Instagram via Facebook Business** (`instagram`) — §4 |
...
27. **Instagram standalone (onda 15) — 2º da família Meta, o molde do Threads aplicado**: (a) OAuth do **Instagram Login** difere do Threads em um salto a mais: o token curto vem de `POST api.instagram.com/oauth/access_token` (endpoint próprio, **form-urlencoded, não a Graph**), depois vira longo (~60d) por `ig_exchange_token` na `graph.instagram.com`. Sem refresh token separado (o token longo se auto-renova por `ig_refresh_token`), refresh reativo — igual Threads. O `/me` devolve o id no campo **`user_id`** (não `id`), gravado em `channelSettings.userId`. (b) **`requiresMedia:true`** (o IG não tem post só-texto, como o TikTok) — o agendamento barra o vazio. (c) **Story é mídia única de propósito**: não existe carrossel de story, e publicar cada mídia separada (o que o Postiz faz) duplicaria no retry — então story com >1 mídia é **422 antes de publicar qualquer coisa**. Como o `validateMedia` do contrato **não recebe settings**, essa checagem mora no publish (falha tardia rara e honesta), não no agendamento. (d) **Réplicas = comentários** (`/{postId}/comments`, só texto — `validateMedia` barra mídia em itens 1+), o `comment()` do Postiz; `capabilities.threads:true` sem gambiarra. (e) Vídeo único no feed é **REELS**; dentro de carrossel é **VIDEO**; imagem no feed dispensa `media_type` (IMAGE é default). (f) Permalink best-effort (depois do `media_publish` nada lança). (g) **Nome do provider = "Instagram"** (o standalone é o caminho primário p/ o criador BR); a variante via Facebook Business (`instagram`, ainda no roteiro) se diferencia quando entrar. O `instagram` saiu do "Em breve" da UI (o standalone cobre "Instagram" no catálogo). (h) **Analytics/colaboradores/áudio/trial reels** ficaram de fora (settings extras do Postiz) — v1 é feed+carrossel+reel+story+comentário.
...
### ⭐ FOLLOW-UP PRINCIPAL DAS PRÓXIMAS IMPLEMENTAÇÕES — providers da Meta (Facebook Pages · Instagram via Business), lendo a referência do Postiz

> **Threads saiu na onda 11** e **Instagram standalone na onda 15** (os dois mais isolados: OAuth/API próprios, sem Página do Facebook no meio) — juntos já provaram o padrão **container → poll → publish** que Facebook e o Instagram via Business reusam. Falta **Facebook Pages** e o **`instagram` via Facebook Business** (a variante que conecta a conta IG por uma Página — exige `business_management` e sub-contas), que compartilham o MESMO app Meta, o MESMO App Review e a MESMA Business Verification. Ordem: **implementar em Development Mode primeiro, submeter depois** — o gate externo NÃO bloqueia o código (ver "Gate legal" abaixo).
...
| ~~`instagram-standalone`~~ ✅ | `instagram.standalone.provider.ts` (236 l.) | `instagram_business_basic`, `instagram_business_content_publish`, `instagram_business_manage_comments`, `instagram_business_manage_insights` | **Entregue na onda 15** — `packages/providers/src/instagram-standalone`. Sobrou: analytics (`instagram_manage_insights`), refresh proativo, colaboradores/áudio/trial reels |
...
6. **Previews**: `network-preview.tsx` já tem o layout de feed do **Instagram** (`InstagramPreview` — mídia quadrada, 1/N do carrossel, legenda com @handle, réplicas como comentários; a variante via Business reusa) e o do **Threads**. Falta só o do **Facebook**, que ainda cai no cartão neutro.
...
tem e entraram na onda 12 (o owner adicionou os ícones). **11 prontas** — 12 providers, porque o
Discord tem dois (Mastodon, Bluesky, Telegram, Discord ×2, LinkedIn, X, TikTok, Threads, **Instagram
standalone**, Twitch, Kick) — e **9 na fila** — a ordem, o gate e o esforço
...
Facebook → Dev.to → Slack → YouTube → Pinterest → Reddit → Dribbble → Medium
(+ Instagram via Facebook Business e Google Business Profile fora da onda / fase 3).
...
| Instagram standalone (família Meta) | 2026-07-23 | analytics (`instagram_manage_insights`), **refresh proativo** do token de 60 dias, colaboradores/áudio/trial reels (settings extras do Postiz), a variante `instagram` via Facebook Business, submissão do App Review; smoke real de publicação com mídia exige túnel HTTPS + **driver S3/R2** |
