...
**Para o manypost:** o kanban por estado (Draft → Aguardando aprovação → Agendado → Publicado/Erro) é **design original nosso**, reusando os estados que o Postiz já modela; aprovação avançada e governança também são originais e vivem no mesmo monorepo — no serviço gerenciado ficam atrás de gate de plano, no self-hosted são liberadas.
...
**Avaliação:** suficiente para "analytics por canal", limitado para comparação/benchmarking (não persiste séries históricas — cada consulta re-busca). No manypost: seguimos essa direção (fetch on-demand por provider + cache) e acrescentamos, como implementação original, benchmarking/histórico/alertas — abertos no repositório, gateados por plano só no gerenciado.
...
## 6. O que segue a direção do Postiz e o que é implementação original

> **Nota (adendo v1.2, 2026-07-17):** a redação original desta seção falava em "núcleo aberto vs.
> premium fechado", desenho que foi **abandonado**. Hoje tudo — inclusive IA operacional,
> governança e billing — vive no mesmo monorepo sob AGPL-3.0; o que existe é gate **comercial**
> por plano no serviço gerenciado, não fronteira de licença. Ver
> [DECISIONS §15-16](DECISIONS.md#adendo-v12-2026-07-17--monorepo-único-100-open-source-estratégia-postiz).
> A distinção que continua valendo é **de autoria**: o que deriva do Postiz (e portanto exige
> atribuição, §8) e o que é implementação original nossa.

**Derivado da direção do Postiz — exige atribuição:**
...
**Implementação original do manypost — mesmo monorepo, mesma licença, sem atribuição devida:**
...
- Billing e admin do serviço gerenciado.
- *Regra de projeto:* essas áreas não referenciam estrutura nem código do Postiz — consomem só os
  contratos internos da aplicação. Isso mantém a fronteira de **autoria** clara mesmo com tudo no
  mesmo repositório.
...
**Nesta pasta:** [Decisões](DECISIONS.md) · [Planos](PLANS.md) · [Gates das plataformas](platform-gates.md) · [Setup das redes](INTEGRATIONS_SETUP.md) · [STATUS](STATUS.md) · [Histórico das ondas](CHANGELOG_ONDAS.md)
